...
def user_aggs():
    try:
        ...
        sink_page_user_count_ddl = f"""
                create table sink_user_page_count(
                    C1 VARCHAR,
                    user_id VARCHAR,
                    C2 VARCHAR,
                    page_id VARCHAR,
        ...
        t_env.execute_sql(sink_page_user_count_ddl)

        t_env.sql_query(
            "SELECT 'USER_ID -->' ,user_id, 'PAGE_ID -->' , page_id , 'COUNT-->', COUNT(*) AS USER_PAGE_COUNT \
             FROM events_table \
             WHERE  TO_TIMESTAMP(visit_timestamp,'yyyy-MM-dd HH:mm:ss') >= TIMESTAMPADD(DAY,-7, CURRENT_TIMESTAMP) \
             GROUP BY user_id,page_id ").execute_insert("sink_user_page_count").wait()
